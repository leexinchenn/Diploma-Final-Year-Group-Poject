...
<body>
	...
	<h1>Product Information : CAKE</h1>
	<hr/>
	
	<div id="add">
		<a href="A.Add Cake Form.php">
			<button>
		...
		<?php
			...
			if(mysqli_num_rows($result)>0) //if there are data
			{
				while($row=mysqli_fetch_array($result))
				{
					?>
					<tr>
						<td style="padding-left:5px;"><?php echo $row["cake_ID"]; ?></td>
						<td><?php echo $row["cake_name"]; ?></td>
						<td><?php echo $row["cake_img_name"]; ?></td>
						<td style="padding-left:5px;"><?php echo number_format($row["cake_price"], 2); ?></td>

						<td>
							<div id="edit">
								<a href="A.Edit Cake Form.php?edit&id=<?php echo $row["cake_ID"];?>">
								<button>
						...
						<td>
							<div id="dlt">
								<a href="A.Info Cake.php?del&id=<?php echo $row["cake_ID"];?>" onclick="return confirmation();">
								<button>
...
<?php
	...
	{
		$id = $_REQUEST["id"]; 
		mysqli_query($connect, "DELETE FROM menu_cake WHERE cake_ID=$id");
