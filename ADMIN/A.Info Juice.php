...
<body>
	...
	<h1>Product Information : JUICE</h1>
	<hr/>
	
	<div id="add">
		<a href="A.Add Juice Form.php">
			<button>
		...
		<?php
			...
			if(mysqli_num_rows($result)>0) //if there are data
			{
				while($row=mysqli_fetch_array($result))
				{
					?>
					<tr>
						<td style="padding-left:5px;"><?php echo $row["juice_ID"]; ?></td>
						<td><?php echo $row["juice_name"]; ?></td>
						<td><?php echo $row["juice_img_name"]; ?></td>
						<td style="padding-left:5px;"><?php echo number_format($row["juice_price"], 2); ?></td>

						<td>
							<div id="edit">
								<a href="A.Edit Juice Form.php?edit&id=<?php echo $row["juice_ID"];?>">
								<button>
						...
						<td>
							<div id="dlt">
								<a href="A.Info Juice.php?del&id=<?php echo $row["juice_ID"];?>" onclick="return confirmation();">
								<button>
...
<?php
	...
	{
		$id = $_REQUEST["id"]; 
		mysqli_query($connect, "DELETE FROM menu_juice WHERE juice_ID=$id");
