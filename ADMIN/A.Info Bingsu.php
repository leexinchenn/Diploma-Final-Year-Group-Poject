...
<body>
	...
	<h1>Product Information : BINGSU</h1>
	<hr/>
	
	<div id="add">
		<a href="A.Add Bingsu Form.php">
			<button>
		...
		<?php
			...
			if(mysqli_num_rows($result)>0) //if there are data
			{
				while($row=mysqli_fetch_array($result))
				{
					?>
					<tr>
						<td style="padding-left:5px;"><?php echo $row["bingsu_ID"]; ?></td>
						<td><?php echo $row["bingsu_name"]; ?></td>
						<td><?php echo $row["bingsu_img_name"]; ?></td>
						<td style="padding-left:5px;"><?php echo number_format($row["bingsu_price"], 2); ?></td>

						<td>
							<div id="edit">
								<a href="A.Edit Bingsu Form.php?edit&id=<?php echo $row["bingsu_ID"];?>">
								<button>
						...
						<td>
							<div id="dlt">
								<a href="A.Info Bingsu.php?del&id=<?php echo $row["bingsu_ID"];?>" onclick="return confirmation();">
								<button>
...
<?php
	...
	{
		$id = $_REQUEST["id"]; 
		mysqli_query($connect, "DELETE FROM menu_bingsu WHERE bingsu_ID=$id");
