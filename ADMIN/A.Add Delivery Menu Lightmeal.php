...
<body>
	...
	<h1>Add Delivery Menu : LIGHTMEAL</h1>
	<hr/>
		...
		<?php
			...
			if(mysqli_num_rows($result)>0) //if there are data
			{
				while($row=mysqli_fetch_array($result))
				{
					...
					<div id="box">
						<div id="pic">
							<?php echo"";?><img src="<?php echo $row["lmeal_img_name"]; ?>"/> <?php echo"<br>";?>
						</div>
						<div id="text">
							<h3><?php echo $row["lmeal_name"]; ?> </h3>
							<!--<p>RM &nbsp;<?php echo $row["lmeal_price"]; ?></p>-->
							<div id="add"><p>
								<a href="A.Add Delivery Menu Lightmeal.php?addlmeal&id=<?php echo $row["lmeal_ID"];?>">
								<button>ADD</button>
								</a>
							</p></div>
						</div>
...
<?php
	if(isset($_REQUEST["addlmeal"]))
	{
		$id = $_REQUEST["id"];
		
		$duplicate=mysqli_query($connect, "SELECT * FROM menu_prod_lightmeal WHERE plmeal_ID=$id");
		...
		if(mysqli_num_rows($duplicate)==0)
		{
			...
			if(mysqli_num_rows($result)==1)
			{
				...
				$add="INSERT INTO menu_prod_lightmeal(plmeal_ID, plmeal_name, plmeal_img_name, plmeal_price) VALUES('$id', '$name', '$img', '$price')";
