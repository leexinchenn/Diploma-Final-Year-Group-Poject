...
<body>
	...
	<h1>Product Information : LIGHTMEAL</h1>
	<hr/>
	
	<div id="add">
		<a href="A.Add Lightmeal Form.php">
			<button>
	...
	<table id="table">
		...
		<?php
			$data="SELECT * FROM menu_lightmeal";
			...
			if(mysqli_num_rows($result)>0) //if there are data
			{
				while($row=mysqli_fetch_array($result))
				{
					?>
					<tr>
						<td style="padding-left:5px;"><?php echo $row["lmeal_ID"]; ?></td>
						<td><?php echo $row["lmeal_name"]; ?></td>
						<td><?php echo $row["lmeal_img_name"]; ?></td>
						<td style="padding-left:5px;"><?php echo number_format($row["lmeal_price"], 2); ?></td>

						<td>
							<div id="edit">
								<a href="A.Edit Lightmeal Form.php?edit&id=<?php echo $row["lmeal_ID"];?>">
								<button>
						...
						<td>
							<div id="dlt">
								<a href="A.Info Lightmeal.php?del&id=<?php echo $row["lmeal_ID"];?>" onclick="return confirmation();">
								<button>
...
<?php
	...
	{
		$id = $_REQUEST["id"]; 
		mysqli_query($connect, "DELETE FROM menu_lightmeal WHERE lmeal_ID=$id");
