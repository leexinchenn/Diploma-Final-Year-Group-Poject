...
<head>
	...
	<style>
		...
		#select{background-color: #FDF5E6; position: -webkit-sticky; position: sticky; top: 0; margin-top: 10px; margin-bottom: 30px; width: 100%; height: auto; float: left;}
		#select ul{list-style-type: none; margin: 0; padding: 0; text-align: center; font-size: 18px;}
		#select ul li{float: left; width: 20%;color: gray;}
		#select ul li:hover{color: gray; background-color:#F2E9D8;}
		#select ul li a{display: block; color: gray; padding: 14px 16px; text-decoration: none; color: gray;}
		#select ul li a:hover{color: black; font-weight: bold; background-color:#F2E9D8;}
		
		h3{font-size: 32px; margin-top:3%; text-align: center;} 
		#coff, #cake, #bingsu, #lmeal, #juice{width:90%; margin-right: auto; margin-left: auto;}
...
<div style="clear:both"></div>		
	...
	<!--<div id="select">
		<ul>
			<li><a href="#coff" >Coffee</a></li>
			<li><a href="#bingsu">Bingsu</a></li>
			<li><a href="#lmeal">Lightmeal</a></li>
	...
	<h3>BINGSU</h3>
	<?php
		$data="SELECT * FROM menu_bingsu ORDER BY bingsu_name ASC";
		$result=mysqli_query($connect, $data);
		...
		if(mysqli_num_rows($result)>0) //if there are data
		{
			...
			{
				?><div id="box"><?php
				echo"";?> <div id="img"><img src="<?php echo $row["bingsu_img_name"]; ?>"/></div>
				<span id="text"><p><?php echo $row["bingsu_name"]."<br><br>";?> </p></span> 
				</div>
				<?php
			}
			?><div style="clear:both"></div><?php
	...
	<h3> LIGHTMEAL </h3>
	<?php
		$data="SELECT * FROM menu_lightmeal ORDER BY lmeal_name ASC";
		$result=mysqli_query($connect, $data);
		...
		if(mysqli_num_rows($result)>0) //if there are data
		{
			...
			{
				?><div id="box"><?php
				echo"";?> <div id="img"><img src="<?php echo $row["lmeal_img_name"]; ?>"/></div>
				<span id="text"><p><?php echo $row["lmeal_name"]."<br><br>";?> </p></span> 
				</div>
				<?php
			}
			?><div style="clear:both"></div><?php
	...
	<?php
		...
		if(mysqli_num_rows($result)>0) //if there are data
		{
			...
			{
				?><div id="box"><?php
				echo"";?> <div id="img"><img src="<?php echo $row["cake_img_name"]; ?>"/> </div>
				<span id="text"><p><?php echo $row["cake_name"]."<br><br>";?> </p></span> 
				</div>
				<?php
			}
			?><div style="clear:both"></div><?php
	...
	<h3>JUICE</h3>
	<?php
		$data="SELECT * FROM menu_juice ORDER BY juice_name ASC";
		$result=mysqli_query($connect, $data);
		...
		if(mysqli_num_rows($result)>0) //if there are data
		{
			...
			{
				?><div id="box"><?php
				echo"";?> <div id="img"><img src="<?php echo $row["juice_img_name"]; ?>"/></div>
				<span id="text"><p><?php echo $row["juice_name"]."<br><br>";?> </p></span> 
				</div>
				<?php
			}
			?><div style="clear:both"></div><?php
